...
const SLACK_WEBHOOK_URL = 'SLACK_WEBHOOK'; //Direccion URL del WebHook donde se envian los datos
...
function openParticipantsInMeet(tabId) {
    chrome.scripting.executeScript({
        target: { tabId: tabId },
        function: () => {
            // Seleccionar el botón para abrir la lista de participantes
            ...
            if (participantsButton) {
                ...
                // Función para esperar a que la lista se cargue
                const waitForParticipantsList = setInterval(() => {
                    // Usamos el nuevo selector para encontrar la lista de participantes
                    const participantsList = document.querySelector(
                        '.ggUFBf.eRa1Wc'  // Este es el selector correcto
                    );

                    if (participantsList && participantsList.scrollHeight > 0) {
                        clearInterval(waitForParticipantsList); // Detener el intervalo
                        console.log("Lista de participantes encontrada y cargada.");

                        // Revisar las dimensiones del contenedor para verificar si hay scroll
                        ...
                        if (scrollHeight > clientHeight) {
                            console.log("Lista tiene barra de scroll. Iniciando desplazamiento...");

                            let scrollStep = 50; // Cantidad de píxeles por paso
                            let scrollPosition = 0;
                            let scrollDirection = 1; // 1: abajo, -1: arriba

                            // Intervalo para realizar el scroll
                            let scrollInterval = setInterval(() => {
                                scrollPosition += scrollStep * scrollDirection;
                                participantsList.scrollTop = scrollPosition;

                                // Si llegamos al final, cambiamos la dirección del scroll
                                if (scrollPosition >= scrollHeight - clientHeight) {
                                    console.log("Llegamos al final. Desplazando hacia arriba...");
                                    scrollDirection = -1;
                                }

                                // Si llegamos al inicio, cambiamos la dirección del scroll
                                if (scrollPosition <= 0) {
                                    console.log("Llegamos al inicio. Desplazando hacia abajo...");
                                    scrollDirection = 1;
                                }
                            }, 100); // Tiempo entre cada paso
                        } else {
                            console.warn("No se necesita desplazamiento, lista completamente visible.");
                        }
                    } else {
                        console.log("Esperando a que la lista de participantes se cargue...");
                    }
...
function monitorStyleChanges() {
    // Selecciona el contenedor específico de los participantes
    const participantsContainer = document.querySelector('.VfPpkd-WsjYwc.VfPpkd-WsjYwc-OWXEXe-INsAgc.KC1dQ.Usd1Ac.AaN0Dd.bJWrn');
    ...
    // Obtiene todos los participantes dentro del contenedor específico
    const targetDivs = participantsContainer.querySelectorAll('[role="listitem"]');  // Lista de todos los participantes
    ...
    const checkStyles = (participants) => {
        participants.forEach(targetDiv => {
            ...
            // Procesa el participante si está hablando o dejó de hablar
            processParticipant(presentationElement, nameElement, speaking);
        });
    };

    // Crea un observador para un grupo de participantes
    const createObserverForGroup = (participantsGroup) => {
    ...
    // Crear observadores de forma eficiente: compartir observadores si es posible
    const groups = [];
    for (let i = 0; i < observerCount; i++) {
        ...
        groups.push(participantsGroup);
    }

    // Asignar observadores a los grupos de participantes
    groups.forEach(group => createObserverForGroup(group));
...
function conditiontoexecute () {

    if (functionEjcute==true){
        chrome.runtime.reload();
        functionEjcute = false;
    }
}
